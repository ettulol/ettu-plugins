...
There are **75 tools**: 5 baseline, 29 read-scoped, and 41 write-scoped. Every HTTP MCP request still requires an authorized ettu OAuth token.
...
| [request_character_angles](#request_character_angles) | `characters:write` | id: UUID; version: integer; expected_revision_id: UUID; expected_version: integer; request_key: UUID |
...
| [retry_character_extra_artwork](#retry_character_extra_artwork) | `characters:write` | id: UUID; version: integer; expected_revision_id: UUID; expected_version: integer; request_key: UUID; artwork_id: UUID |
...
Read an owned version's extra angles and face portrait, including can_request_angles, failures and retry availability. Angles are optional and only start on the owner's request. Extra jobs never change publication or replace the approved picture. Read does not generate. Use get_character_artwork with asset=avatar to retrieve a finished face portrait.
