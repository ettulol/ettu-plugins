...
Read an accessible channel, cast, role, version, ordered episode summaries, and latest_episode with its public video thumbnail preview_url when available. Use get_channel_episode for scenes. Website: /channels/{id}; Watch contains playback, while Studio groups Story, Plan and Activity. Reading previews never generates artwork or publishes anything.
