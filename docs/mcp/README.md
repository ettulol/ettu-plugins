...
The authenticated floating **Chat** button opens a left-side panel beside the app. Navigation stays usable while chatting; minimizing preserves the current draft and streamed reply. **Conversations** opens recent or archived history within the panel, and existing `/chat?conversation={id}` links open the selected conversation there. Signing out or changing accounts clears the mounted private state. Opening, minimizing and navigating never send a message, approve an action or stop a run. Its owner-only API and external MCP tools share the same database commands and checks. Discover `list_assistant_conversations` (recent or archived, `next_offset`) and `get_assistant_conversation` (latest 100 messages, `next_before` for older pages); neither starts AI work. Content consists of user/assistant text and structured tool calls/results. Provider continuation data, leases and internal checkpoints are private to workers and excluded from both interfaces.
